...
public class LibraryServiceService {
    ...
    public List<LibraryItem> getLibraryItemsFromCreator(String creatorName) throws Exception {
        ...
        for (LibraryItem a : allItems) {
            if (a.getCreator().equals(creatorName))
                itemsByCreator.add(a);
        }
    ...
    public List<LibraryItem> getLibraryItemFromCreatorAndTitle(String creatorName, String itemTitle) throws Exception {
        ...
        for (LibraryItem a : allItems) {
            if (a.getCreator().equals(creatorName) && a.getName().equals(itemTitle))
                itemsByCreatorAndTitle.add(a);
        }
     ...
     * @author Ramin Akhavan-Sarraf checked
     */
     ...
     * @returns a newspaper
     * @author Ramin Akhavan-Sarraf I added a throw I think u forgot :) -elo checked
     */
    @Transactional
     ...
     * @returns a newspaper
     * @author Ramin Akhavan-Sarraf * I added a throw I think u forgot :) -elo
     *         checked
     */
    @Transactional
    ...
    /**
     * Creates an item reservation transaction between a user account and a
     * borrowable item
     * 
     * @param item
     * @param account
    ...
    /**
     * Creates a room reservation transaction between a user account and a room
     * (borrowable item)
     * 
     * @param item
     * @param account
    ...
    public Transaction createRoomReserveTransaction(BorrowableItem item, UserAccount account, Date date, Time startTime,
        ...
        Transaction roomReservation = new Transaction(item, account, TransactionType.RoomReservation, date); // No
                                                                                                             // deadline
                                                                                                             // for room
                                                                                                             // reservation
    ...
    /**
     * Creates an item borrow transaction between a user account and a borrowable
     * item
     * 
     * @param item
     * @param account
    ...
    public Transaction createItemBorrowTransaction(BorrowableItem item, UserAccount account) {
        ...
        if (item.getState() != ItemState.Available) {
            error += "This item is not available and cannot be borrowed, please try waitlist! ";
                ...
                || item.getLibraryItem().getType() == ItemType.NewspaperArticle) {
    ...
    /**
     * Creates an item return transaction between a user account and a borrowable
     * item
     * 
     * @param item
     * @param account
     * @return Transaction
     * @author Amani Jammoul cheked
     */
    @Transactional
    ...
    /**
     * Creates an item waitlist transaction between a user account and a borrowable
     * item
     * 
     * @param item
     * @param account
     * @return Transaction
     * @author Amani Jammoul Added a check if item is available or not cheked
     */
    @Transactional
    public Transaction createItemWaitlistTransaction(BorrowableItem item, UserAccount account) {
        ...
        Transaction itemReservation = new Transaction(item, account, TransactionType.Waitlist, null); // No deadline for
                                                                                                      // waitlist
        transactionRepository.save(itemReservation);
        return itemReservation;
    }
    ...
    /**
     * Creates an item renewal transaction between a user account and a borrowable
     * item
     * 
     * @param item
     * @param account
    ...
    /**
     * @param account
     * @return List<BorrowableItem> - list of all items borrowed (checked out) by a
     *         user
     * @author Amani Jammoul checked
     */
    @Transactional
    public List<BorrowableItem> getBorrowedItemsFromUser(UserAccount account) {
        ...
        for (Transaction t : allUserTransactions) {
            if (t.getTransactionType().equals(TransactionType.Borrowing)
                    || t.getTransactionType().equals(TransactionType.Renewal)) {
    ...
    /**
     * @param item
     * @return List<UserAccount> - list of all user accounts that are on the
     *         waitlist for an item
     * @author Amani Jammoul cheked
     */
    @Transactional
    ...
    /**
     * Headlibrarian getters 1.get HeadLibrarian by ID (THROWS EXCPEPTION IF NOT
     * EXIST)
     * 
    ...
    /**
     * Headlibrarian helper Method boolean to ensure only and always 1 head
     * librarian instance at any time
     * 
     * @author Eloyann Roy-Javanbakht
     * 
     */
    ...
    /**
     * Headlibrarian getters get currentheadlibrarian in the system
     * 
     * @author Eloyann Roy-Javanbakht
     * 
     */
    public HeadLibrarian getHeadLibrarian() throws Exception {
        ...
        try {
            ...
            for (HeadLibrarian head : headLibrarian) {
                return head;
            }
        } catch (Exception e) {
            throw new Exception("There isn't any headLibrarian");
     ...
     * @author Eloyann Roy-Javanbakht
     */
    public HeadLibrarian createNewHeadLibrarian(String aFirstName, String aLastName, boolean aOnlineAccount,
            String aAddress, String aPassword, int aBalance, String aEmail) throws Exception {
        ...
        if (headLibrarians.size() > 0)
            throw new Exception("There is already a HeadLibrarian AccountExisting");
    ...
    /**
     * Headlibrarian delete a HeadLibrarian object with user ID
     * 
     * @author Eloyann Roy-Javanbakht
     */
    public HeadLibrarian deleteHeadLibrarian(int userID) throws Exception {
        HeadLibrarian headLibrarian = getHeadLibrarian();
        HeadLibrarian thisone = getHeadLibrarianFromUserId(userID);
        if (thisone.getUserID() == (headLibrarian.getUserID()) == false)
            throw new Exception("The UserID provided does not correspond to a  Head Librarian Account");
    ...
    /**
     * Create a new Librarian Checks 1 : input validity Checks 2 : theres creater is
     * instance of HeadLibrarian before adding this one Checks 3: that this user
     * does not already have an account as a librarian Calls on check if
     * 
     * @author Eloyann Roy-Javanbakht
     * 
     *         CHANGING TO STRING INPUT
     */
    public Librarian createANewLibrarian(int createrID, String aFirstName, String aLastName, boolean aOnlineAccount,
            String aAddress, String aPassword, int aBalance, String aEmail) throws Exception {
        ...
        Librarian librarian = new Librarian(aFirstName, aLastName, aOnlineAccount, aAddress, aPassword, aBalance,
                aEmail);
    ...
    /**
     * Delete an instance of Librarian checks if user has credentials checks if it
     * exists as a librarian
     * 
     * @author Eloyann Roy-Javanbakht
     */
    ...
    /**
     * Librarian getters 1. get librarian from full name
     * 
     * @author Eloyann Roy-Javanbakht
     */
    ...
    /**
     * Librarian getters 1. get librarian from userID name
     * 
     * @author Eloyann Roy-Javanbakht
     */
     ...
     * @return List of TimeSlots cheked
     */
    @Transactional
     ...
     * @return List of timeslots checked
     */
    @Transactional
    ...
    /**
     * Get timeslot list (workshifts) for a specific librarian by inputing the
     * librarian,s UserId
     * 
     * @author Mathieu Geoffroy
    ...
    /**
     * Create a timeslot in the first (and only) available library system with the
     * first (and only) available head librarian. The timeslot is saved to the DB.
     * 
     * @author Mathieu Geoffroy
     ...
     * @throws Exception - If there is no head librarian checked
     */
    @Transactional
    public TimeSlot createTimeSlot(int accountID, Date startDate, Time startTime, Date endDate, Time endTime)
            throws Exception {
        UserAccount account = userAccountRepository.findUserAccountByUserID(accountID);
        if (account == null)
            throw new IllegalArgumentException("Invalid account");
        if (!(account instanceof HeadLibrarian))
            throw new IllegalArgumentException("Only a Head Librarian can create a new timeslot.");
        if (startDate == null)
     ...
     * @return updated TimeSlot added checks -elo checked
     */
    @Transactional
    public TimeSlot assignTimeSlotToLibrarian(int accountId, int timeSlotID, int librarianUserId) {
        ...
        if (account == null)
            ...
            error = error + "This User ID does not correspond to a Head Librarian. ";
    ...
    public boolean deleteTimeSlot(int accountId, int timeslotID) throws Exception {
        ...
        if (account == null)
            ...
            error = error + "This User ID does not correspond to a Head Librarian. ";
        if (timeslotID < 1)
            error = error + "Invalid timeslotID. ";
     ...
     * 
     * @author Mathieu Geoffroy
     * @param day - string for day of week that MUST start with a capital letter
     *            (case sensitive matching)
     * @return list of opening hours of a certain day of week
     * @throws Exception - when the input is not match (case-sensitve) with the
     *                   correct day of week checked
     */
    @Transactional
     ...
     * @return - list of opening hours added some -elo checked
     */
    @Transactional
    ...
    /**
     * Create opening hour with the first (and only) available library system and
     * first (and only) available head librarian. Saves the opening hour to the DB
     * 
     * @author Mathieu Geoffroy
     * @param day       - string of day of week (case-sensistive): MUST start with
     *                  capital letter
     * @param startTime - start time of opening hour
     * @param endTime   - end time of opening hour
     * @return the create opening hour
     * @throws Exception - When the day string does not match the DayOfWeek enum
     *                   format
     * @throws Exception - When there is no library systme
     * @throws Exception - When there is no head librarian cheked
     */
    @Transactional
    ...
    public boolean deleteOpeningHour(int accountId, int openingHourID) throws Exception {
        ...
        if (account == null)
            error = error + "Invalid account. ";
        if (openingHourID < 1)
            error = error + "Invalid openinghourID. ";
        error = error.trim();
     ...
     * @throws Exception - when there is no library system checked
     */
    @Transactional
     ...
     * @return list of holiday checked
     */
    @Transactional
    ...
    /**
     * Create holiday with first (and only) available library system and the first
     * (and only) avaialbe head librarian. Saves to DB
     * 
     * @author Mathieu Geoffroy
     ...
     * @throws Exception - when there is no library system
     * @throws Exception - when there is no head librarian checked
     */
    @Transactional
    public Holiday createHoliday(int accountId, Date date, Time startTime, Time endTime) throws Exception {
        ...
        UserAccount account = userAccountRepository.findUserAccountByUserID(accountId);
        if (!(account instanceof HeadLibrarian))
            throw new IllegalArgumentException("Account creating the holiday must be a head librarian.");
        if (date == null)
     ...
     */
    @Transactional
    public LibraryItem createLibraryItem(String name, String itemType, Date date, String creator, boolean isViewable)
            throws Exception {
        String error = "";
    ...
    /***
     * This method gets the patron object, given the userID, from a list of patrons
     * in the database.
     * 
     * @author Zoya Malhi
     * @param firstName, lastName
     * @return null added checks -elo checked
     * @throws Exception
     */
    @Transactional
    public Patron getPatronFromFullName(String firstName, String lastName) throws Exception {
        ...
        if (patron != null)
            ...
            throw new IllegalArgumentException("No patron found with this name! ");
     ...
     * 
     * @author Zoya Malhi
     * @param head, aFirstNAme, aLastName, aOnlineAccount, aAddress,
     *              aValidatedAccount, aPassword, aBalance
     * @return patron added checks -elo edited by Gabby checked
     */
    @Transactional
    ...
    /**
     * @author Gabrielle Halpin This method allows the user to change their
     *         firstName
     * @param aFirstName
     * @param userID
    ...
    /**
     * @author Gabrielle Halpin This method allows the user to change their address
     * @param aAddress
     * @param userID
     * @return Useraccount account
     */
    ...
    /**
     * @author Gabrielle Halpin This method allows the user to change their email
     * @param aEmail
     * @param userID
     * @return Useraccount account
     */
    ...
    /**
     * This mathod is called when the Librarian set's a customer's account to an
     * online account
     * 
     ...
     * @return UserAccount
     */
    public UserAccount setOnlineAccount(int userID, String aEmail, String aPassword, boolean aOnlineAccount,
            int creatorID) {
        if (userID <= 0) {
            throw new IllegalArgumentException("Invalid ID");
        }
    ...
    /**
     * This method sets the validity of the user account which must be done by a
     * librarian
     * 
     * @author Gabrielle Halpin
     * @param patronID
